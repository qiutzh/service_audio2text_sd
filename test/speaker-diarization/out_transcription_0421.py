...
def main(args):
    ...
    rec_ids = list(A.keys())[rank::threads_num]  # 切片,每隔threads_num个元素取一个
        ...
        try:
            ...
            with open(rttm_path, 'r') as f:
                for i in f.readlines():
                    i = i.strip().split()  # [start, end, speaker_id]
                    output_field_labels.append([float(i[3]), float(i[3]) + float(i[4]), i[7]])
            sentence_info = get_trans_sentence(A[rec_id], asr_pipeline)  # todo
...
if __name__ == "__main__":
    ...
    args.exp_dir = "/home/rise/qiutzh/service-audio2text-sd/exp"
    args.gpu = "0"
    main(args)
