...
# from app.open_toolkit.speakerlab.utils.config import build_config
...
def make_rttms(seg_list, out_rttm, rec_id):
    ...
    with open(out_rttm, 'w') as f:
        for seg in new_seg_list:
            ...
            f.write(line_str.format(seg_id, seg_st, seg_ed - seg_st, cluster_id))  # todo - 这里存的是时间间隔
...
def audio_only_func(local_wav_list, audio_embs_dir, rttm_dir, config):
    ...
    for wav_file in local_wav_list:
        ...
        if not os.path.exists(embs_file):
            print(
                "[WARNING]: %s does not exist, it is possible that vad model did not detect valid speech in file %s, please check it." % (
                embs_file, wav_file))
            continue
...
def main():
    ...
    args.conf = "/home/rise/qiutzh/service-audio2text-sd/conf/diar.yaml"
    args.wavs = "/home/rise/qiutzh/service-audio2text-sd/data/examples/2speakers_example.wav"
    args.audio_embs_dir = "/home/rise/qiutzh/service-audio2text-sd/exp/embs"
    args.rttm_dir = "/home/rise/qiutzh/service-audio2text-sd/exp/rttm/intermediate"
    wav_list = []
    if args.wavs.endswith('.wav'):
        # input is a wav path
        wav_list.append(args.wavs)
    else:
