...
class Diarization3Dspeaker():
    ...
    def __init__(self, device=None, include_overlap=False, hf_access_token=None, speaker_num=None,
        ...
        if include_overlap:
            self.segmentation_model = get_segmentation_model(hf_access_token,
                                                             self.device,
                                                             f"{model_cache_dir}/pyannote")
...
def init_dia_model(device, include_overlap, hf_access_token, speaker_num, model_cache_dir):
    dia_model = Diarization3Dspeaker(device, include_overlap, hf_access_token, speaker_num, model_cache_dir)
    return dia_model
...
class SpeakerDiarizationInference(object):
    def __init__(self,
                 ...
                 nprocs=1):
        # self.audio__file = audio_file  # 输入音频，填入文件夹，格式支持wav
        self.hf_access_token = hf_access_token  # hf_access_token for pyannote/segmentation-3.0 model
        self.speaker_num = speaker_num  # 说话人数量(先验信息)
        self.include_overlap = include_overlap  # 是否包含重叠区域
        self.disable_pogress_bar = disable_pogress_bar  # 显示
        self.nprocs = nprocs  # 进程数

        self.out_type = "rttm"  # 说话人分离结果保存格式，rttm 或 json
        self.out_dir = "/home/rise/qiutzh/service-audio2text-sd/exp_infer"  # 输出文件夹
        ...
        self.dia_model = init_dia_model(self.device,
                                        self.include_overlap,
                                        self.hf_access_token,
                                        self.speaker_num,
                                        self.checkpoint_dir)
        self.dia_result = []  # 说话人分离识别结果

    def model_initialize(self):
        get_speaker_embedding_model(self.device, self.checkpoint_dir)
        get_voice_activity_detection_model(self.device, self.checkpoint_dir)
        get_cluster_backend()
        if self.include_overlap:
            get_segmentation_model(self.hf_access_token, self.device, f"{self.checkpoint_dir}/pyannote")
    ...
    def infer(self, audio_file):
        ...
        if audio_file.endswith('.wav') or audio_file.endswith('.mp3'):
            ...
            try:
                with open(audio_file, 'r') as f:  # input should be a wav list
                    wav_list = [i.strip() for i in f.readlines()]
            except Exception as err:
                raise Exception('[ERROR]: Input should be a wav[mp3] file or a wav[mp3] list.')
...
def main():
    ...
    # wav = "/home/rise/qiutzh/service-audio2text-tools/data/audio_tempdata_0418/35ce90cc-0b75-11ef-84d9-4f9fb6367409.wav"
    # wav = "/home/rise/qiutzh/service-audio2text-tools/data/audio_tempdata_0418/4b2568d4-ab93-11ee-9b5d-b30f2649e394.wav"
    wav = "/home/rise/qiutzh/service-audio2text-tools/data/audio_tempdata_0418/b6b4bb28-d153-11ee-a758-d71e97cf9d82.wav"
    # wav = "/home/rise/qiutzh/service-audio2text-sd/data/examples/wav.list.20"
    sd_infer_model = SpeakerDiarizationInference()
    dia_result = sd_infer_model.infer(wav)
    print(dia_result)
