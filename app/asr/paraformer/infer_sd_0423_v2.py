...
class SpeakerDiarizationInference(object):
    ...
    def model_initialize(self):
        ...
        get_cluster_backend()
        if self.include_overlap:
            get_segmentation_model(self.hf_access_token)
        print(f'[INFO]: Model downloaded successfully.')

    def infer_step(self, rank, nprocs, wav_list):
        if not torch.cuda.is_available():
            device = torch.device('cpu')
        else:
            ngpus = torch.cuda.device_count()
            device = torch.device('cuda:%d' % (rank % ngpus))
        dia_model = Diarization3Dspeaker(device, self.include_overlap, self.hf_access_token, self.speaker_num)

        wav_list = wav_list[rank::nprocs]
        if rank == 0 and (not self.disable_pogress_bar):
            wav_list = tqdm(wav_list, desc=f"Rank 0 processing")
        for wav_path in wav_list:
            dia_output = dia_model(wav_path)
            self.dia_result.append({wav_path: dia_output})
            # 缓存到本地文件
            wav_id = os.path.basename(wav_path).rsplit('.', 1)[0]
            if self.out_dir is not None:
                out_file = os.path.join(self.out_dir, wav_id + '.%s' % self.out_type)
            else:
                out_file = '%s.%s' % (wav_path.rsplit('.', 1)[0], self.out_type)
            dia_model.save_diar_output(out_file, wav_id)
    ...
    def infer(self, audio_file):
        ...
        if self.nprocs is None:
            ...
            if ngpus > 0:
                ...
                self.nprocs = 1
        self.nprocs = min(len(wav_list), self.nprocs)
        print(f'[INFO]: Set {self.nprocs} processes to extract embeddings.')

        self.dia_result = []  # 说话人分离识别结果
        mp.spawn(self.infer_step, nprocs=self.nprocs, args=(self.nprocs, wav_list))
...
def infer_demo():
    ...
    sd_infer_model.infer(wav)
    print(sd_infer_model.dia_result)
