...
class SpeakerDiarizationInference(object):
    def __init__(self,
        ...
        self.wav_list = []
        if isinstance(self.audio__file, list):
            self.wav_list = self.audio__file
        elif isinstance(self.audio__file, str):
            self.wav_list = [self.audio__file]
            if self.disable_pogress_bar:
                self.wav_list = tqdm(self.wav_list, desc=f"Rank 0 processing")

        self.nprocs = min(len(self.wav_list), nprocs)
        print(f'[INFO]: Set {self.nprocs} processes to extract embeddings.')

        self.dia_model = Diarization3Dspeaker(self.device, self.include_overlap, self.hf_access_token, self.speaker_num)
    ...
    def infer(self):
        ...
        for wav_path in self.wav_list:
            dia_output = self.dia_model(wav_path)
            # 数据写入
            ...
            self.dia_model.save_diar_output(out_file, wav_id)
        return dia_output
...
def infer_demo():
    ...
    hf_access_token = ""
    sd_infer_model = SpeakerDiarizationInference(wav, hf_access_token)
    result = sd_infer_model.infer()
    print(result)
